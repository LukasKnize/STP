...
let projects = document.querySelectorAll(".footerCarousel");
setInterval(() => {
    projects[currentImg].style.display = "block";
    if (currentImg == 0) {
        projects[projects.length - 1].style.display = "none"; 
    }else{
        projects[currentImg - 1].style.display = "none";
    }
    if (currentImg == projects.length - 1) {
        currentImg = 0;
    } else {
        currentImg += 1;
    }
